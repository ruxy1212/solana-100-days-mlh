...
Open the Day 73 vault program in [lib.rs](../day73/lib.rs). Break the signer seed or bump used for the `withdraw` CPI, rebuild, and run the vault test in [vault.ts](../day73/vault.ts). Capture the log line that points to the missing or invalid PDA signature.

### 3. Wrong Account Constraint
Restore the Day 73 vault program. Then add or change an account constraint in the Day 74 callee so the caller no longer satisfies it, rebuild, and run the Day 74 test in [compose-lab.ts](../day74/compose-lab.ts). Capture the log line that names the failing constraint or account.
